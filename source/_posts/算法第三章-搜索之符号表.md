---
title: 算法第三章-搜索之符号表
date: 2018-03-08 10:08:09
tags: [算法]
categories: 算法
permalink: algorithm-searching-SymbolTables
---

> 定义：符号表是一种存储键值对的数据结构，支持两种操作：插入（`put`），即将一组新的键值对存入表中；查找（`get`），即根据给定的键得到相应的值。

简单的泛型符号表的API如下图所示：
![stAPI](/uploads/algorithm/searching/st-api.png)
...
![sequential-search](/uploads/algorithm/searching/sequential-search.png)
...
![binary-search](/uploads/algorithm/searching/binary-search.png)
